# netsupport_monitor_simple.py - Simpele Tray + CMD Interface
import sys
import os
import json
import threading
...
def show_main_screen():
    ...
    print("|                                                    |")
    print(f"| Laatste check: {get_last_check():<34} |")
    ...
    print("|                                                    |")
    print("+======================================================+")
    print("| [S] Settings  [L] Logs  [R] Refresh  [Q] Sluiten  |")
    print("+======================================================+")
    print()
...
def change_detection_method(config):
    ...
    if choice in methods:
        ...
        save_config(config)
        print(f"Method gewijzigd naar: {methods[choice]}")
        time.sleep(1)
    ...
    while True:
        ...
        if choice == 's':
            ...
            if os.path.exists("netsupport_log.txt"):
                ...
                print("Geen log bestand gevonden.")
                print("Schakel logging in via Settings.")
...
'''
    ...
    def get_status_text(self):
        ...
        if getattr(self.monitor, 'last_status', False):
            return "[WARNING] LEERKRACHT VERBONDEN"
        return "[SAFE] VEILIG - Geen verbinding"
    ...
    def show_settings_window(self):
        ...
        # Create a settings-focused script
        script_content = '''
import json
import os
import sys
...
def show_settings():
    ...
    while True:
        ...
        print("|                                                    |")
        print(f"| 1) Detection Method: {config.get('detection_method', 'process'):<26} |")
        print(f"| 2) Scan Interval: {config.get('scan_interval', 2)} seconds{' '*(24)} |")
        logging_status = "On" if config.get('logging', False) else "Off"
        print(f"| 3) Logging: {logging_status:<37} |")
        print("|                                                    |")
        print("| [1-3] Change setting                              |")
        print("| [S] Save and close                                |")
        ...
        if choice == '1':
            ...
            if method_choice in methods:
                ...
                save_config(config)
                print(f"Method changed to: {methods[method_choice]}")
                time.sleep(1)
            ...
            try:
                ...
                if 1 <= interval <= 10:
                    ...
                    print("Invalid interval (must be 1-10)")
                    time.sleep(1)
            except ValueError:
                print("Invalid input")
                time.sleep(1)
...
if __name__ == "__main__":
    os.system("title NetSupport Monitor - Settings")
    os.system("color 0A")
    show_settings()
'''
        ...
        try:
            ...
            batch_script = f'''@echo off
title NetSupport Monitor - Settings
color 0A
...
'''
            ...
            print(f"Could not open settings window: {e}")

    def show_statistics_window(self, stats):
        """Show statistics in CMD window"""
        ...
        if last_conn:
            try:
                ...
                if 0 <= last_conn <= 4102444800:  # Jan 1, 2100
                    ...
                    last_conn_str = 'Invalid timestamp'
        ...
        # Create statistics display script with safe string interpolation
        script_content = f'''
import os
import sys
...
def show_stats():
    ...
    print("+======================================================+")
    print("|                                                    |")
    print(f"| Total connections:     {{total:<27}} |")
    print(f"| Connections today:     {{today:<27}} |")
    print(f"| Total duration:        {{duration_str}} minutes{{' '*(27-len(duration_str)-8)}} |")
    print("|                                                    |")
    print(f"| Last connection:                                   |")
    print(f"|   {{last_conn:<50}} |")
    print("|                                                    |")
    print(f"| Recent history:        {{history}} entries{{' '*(27-len(history)-8)}} |")
    print("|                                                    |")
    print("+======================================================+")
    print("|                                                    |")
...
if __name__ == "__main__":
    os.system("title NetSupport Monitor - Statistics")
    os.system("color 0A")
    show_stats()
'''
        ...
        try:
            ...
            batch_script = f'''@echo off
title NetSupport Monitor - Statistics
color 0A
...
'''
            ...
            print(f"Could not open statistics window: {e}")
...
class NetSupportMonitorSimple:
    ...
    def setup_auto_start(self, enable: bool):
        ...
        if not WINDOWS_AVAILABLE:
            print("Auto-start only available on Windows")
            return False
        ...
        try:
            ...
            if self.config.get("silent_mode", False):
                ...
                menu = pystray.Menu(
                    ...
                    pystray.MenuItem("⚙️ Settings", self.show_simple_settings),
                    pystray.MenuItem("📈 Statistics", self.show_statistics),
                    pystray.Menu.SEPARATOR,
                    pystray.MenuItem("🔔 Notifications", self.toggle_notifications,
    ...
    def reload_config(self):
        ...
        try:
            ...
            if old_config.get("detection_method") != self.config.get("detection_method"):
                print(f"Detection method changed to: {self.config.get('detection_method')}")

            # Check if scan interval changed
            if old_config.get("scan_interval") != self.config.get("scan_interval"):
                print(f"Scan interval changed to: {self.config.get('scan_interval')}s")
    ...
    def exit_program(self):
        """Clean shutdown"""
        print("Shutting down NetSupport Monitor...")
    ...
    def run(self):
        ...
        print("=== NetSupport Monitor v3.0 - Simpele Interface ===")
        ...
        print("✅ NetSupport Monitor draait in system tray")
        print("🖱️  Rechtsklik op tray icon voor opties")
